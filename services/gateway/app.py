"""
API Gateway – proxies API calls and serves frontend + static assets.
"""
import os
import sys
import time
...
import requests
from flask import Flask, request, jsonify, Response, render_template, g, send_from_directory
...
def _proxy(base_url: str, path: str = ""):
    ...
    for attempt in range(PROXY_RETRIES + 1):
        try:
            resp = requests.request(method=request.method, url=url, headers=headers, data=request.get_data(), timeout=PROXY_TIMEOUT)
    ...
    return jsonify({"error": last_err or "upstream unavailable", "upstream": base_url}), 503

@app.get("/health")
def health():
    services = {}
    overall = "ok"
    for name, url in [("auth", AUTH_URL), ("destinations", DEST_URL), ("recommendations", REC_URL), ("itineraries", ITIN_URL)]:
        try:
